...
(defun ensure-path-options-keys (path options)
  (destructuring-bind (&key copy link) options
    (when (and copy link)
      (error "Only one of :COPY or :LINK can be specified for a path.~%Path: ~s." path))))

(defun ensure-path-single-transform (components path)
  (when (> (count 'transform components :key #'car) 1)
    (error "Cannot have multiple transform components per path.~%Path: ~s." path)))
...
(defun ensure-copy/link-source-valid (path source)
  (loop :for x = #\Nul :then y
        :for y :across source
        :when (char= x y #\/)
          :do (error "The source of a :LINK or :COPY cannot contain adjacent \"/\" characters.~%~
                      Path: ~s." path)))
...
(defun ensure-component-form (component path)
  (unless (and (> (length component) 1)
    ...
    (error "Component form:~%~s~%must be a list of the form: (TYPE (OPTIONS) ARGS).~%Path: ~s"
           component path)))
...
(defun ensure-component-type-symbol (type path)
  (unless (and (symbolp type)
               (not (keywordp type)))
    (error "Component type ~s must be a non-keyword symbol.~%Path: ~s" type path)))
...
(defun ensure-component-args-valid (type args path)
  (loop :with valid-args = (compute-component-initargs type)
        ...
        :unless (member key valid-args)
          :do (error "Component type ~s has an invalid argument: ~s.~%Path: ~s." type key path)))

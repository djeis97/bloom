     ...
     (ecase (resolve-uniform-type uniform-type)
       (:sampler
        (let ((unit (texture-unit-state material)))
          ...
          (lambda (uniform value)
            (gl:active-texture unit)
            (gl:bind-texture (au:href +sampler-targets+ uniform-type) value)
            (shadow:uniform-int uniform unit))))
       ((:bool :int) #'shadow:uniform-int)
       (:float #'shadow:uniform-float)
       (:vec2 (lambda (uniform value)
                (shadow:uniform-vec2 uniform (m:get-array value))))
       (:vec3 (lambda (uniform value)
                (shadow:uniform-vec3 uniform (m:get-array value))))
       (:vec4 (lambda (uniform value)
                (shadow:uniform-vec4 uniform (m:get-array value))))
       (:mat2 (lambda (uniform value)
                (shadow:uniform-mat2 uniform (m:get-array value))))
       (:mat3 (lambda (uniform value)
                (shadow:uniform-mat3 uniform (m:get-array value))))
       (:mat4 (lambda (uniform value)
                (shadow:uniform-mat4 uniform (m:get-array value))))))
    (cons
         ...
         (:sampler
          (let* ((unit-list (au:iota dimensions :start (texture-unit-state material)))
            ...
            (lambda (uniform value)
              ...
              (shadow:uniform-int-array uniform unit-array))))
...
(defmacro with-material ((material) &body body)
  `(shadow:with-shader-program (shader ,material)
     (au:do-hash (name uniform (uniforms ,material))
       (funcall (binder uniform) name (actual-value uniform)))
     ,@body))
